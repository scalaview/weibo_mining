...
class Sina_master():
  ...
  def __init__(self,consumer_key,consumer_secret, dao=None):
      self.consumer_key,self.consumer_secret, self.dao = consumer_key,consumer_secret, dao
...
#            
  ...
  def get_latest_weibo(self,user_id,count):
      ...
      for line in timeline:
          ...
          statusprofile['created_at'] = self.getAtt("created_at")
          statusprofile['text'] = self.getAtt("text")
          # unicode(self.getAtt("text"), 'utf-8')
          # print "test:%s" %(self.getAtt("text").encode('utf8'))
          statusprofile['source'] = self.getAtt("source")
          statusprofile['favorited'] = self.getAtt("favorited")
          statusprofile['truncated'] = self.getAtt("ntruncatedame")
          statusprofile['in_reply_to_status_id'] = self.getAtt("in_reply_to_status_id")
          ...
          statusprofile['bmiddle_pic'] = self.getAtt("bmiddle_pic")
          statusprofile['original_pic'] = self.getAtt("original_pic")
          statusprofile['geo'] = self.getAtt("geo")#repr(pickle.dumps(self.getAtt("geo"),pickle.HIGHEST_PROTOCOL))
          statusprofile['mid'] = self.getAtt("mid")
...
#                print i,j[i]
  ...
  def save_data(self, statuses):
  	if self.dao is not None:
  		for s in statuses:
  			self.dao.insert_or_update(s)

def reptile(sina_reptile,userid):
  ids_num,ids,new_ids,return_ids = 1,[userid],[userid],[]
  while(ids_num <= conf.crawled_count):
      next_ids = []
      for id in new_ids:
          try:
              ...
              if statuses is None:
                  continue
              sina_reptile.save_data(statuses)
          except Exception as e:
          ...
          ids_num+=1
          print ids_num
          if(ids_num >= conf.crawled_count):break
...
def run_crawler(consumer_key,consumer_secret,key,secret,userid):
  try:
			dao = WebDao(conf.db_name)
			sina_reptile = Sina_master(consumer_key,consumer_secret, dao)
